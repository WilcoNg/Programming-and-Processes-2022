...
def delOne(title, subject, topic, date_set, date_due):
    ...
    listUpdate(title, subject, topic, date_set, date_due)
    cur.execute('delete from assignments where title = ? and subject = ? and topic = ? and date_set = ? and date_due = ?', (title, subject, topic, date_set, date_due))
    t1.delete(val)
    t2.delete(val)
    t3.delete(val)
    t4.delete(val)
...
b2 = ttk.Button(root, text='Delete', width=20, command=lambda:delOne(e1.get(), e2.get(), e3.get(), e4.get(), e5.get()))
