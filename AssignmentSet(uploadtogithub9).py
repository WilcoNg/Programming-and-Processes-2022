...
class MainApp(Tk):
    def __init__(self, *kw, **kwarg):
        ...
        self.e2 = ttk.OptionMenu(self.frame, self.subjectoption, *self.Subjects)
        self.e3 = ttk.Entry(self.frame, width=20)
        self.e4 = ttk.Entry(self.frame, width=20)
        self.e5 = ttk.Entry(self.frame, width=20)

        self.t1 = Listbox(self.frame, exportselection=False, width=20, height=20, selectmode='SINGLE')
        self.t2 = Listbox(self.frame, exportselection=False, width=20, height=20, selectmode='SINGLE')
        self.t3 = Listbox(self.frame, exportselection=False, width=20, height=20, selectmode='SINGLE')
        self.t4 = Listbox(self.frame, exportselection=False, width=20, height=20, selectmode='SINGLE')
        self.t5 = Listbox(self.frame, exportselection=False, width=20, height=20, selectmode='SINGLE')

        self.b1 = ttk.Button(self.frame, text='Add assignment', width=20, command=self.addassignment)
        self.b2 = ttk.Button(self.frame, text='Delete', width=20, command=lambda:self.delOne(self.e1.get(), self.subjectoption.get(), self.e3.get(), self.e4.get(), self.e5.get()))
        self.b3 = ttk.Button(self.frame, text='Delete all', width=20, command=self.deleteAll)
        self.b4 = ttk.Button(self.frame, text='Exit', width=20, command=self.bye)
    ...
    def delOne(self, title, subject, topic, date_set, date_due):
        try:
            ...
            cur = conn.cursor()
            val = self.t1.curselection() or self.t2.curselection() or self.t3.curselection() or self.t4.curselection() or self.t5.curselection()
